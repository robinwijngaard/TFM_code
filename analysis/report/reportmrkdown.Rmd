...
samplecnv <- subset(cnvFounds_exomedepth, cnvFounds_exomedepth$sample == params$sample)
panelannotated <- subset(annotatedFile, annotatedFile$panell == params$panell)

setwd(reportDir)
write.table(samplecnv, file.path(reportDir, "samplecnv.bed"), sep="\t", row.names=FALSE, quote = FALSE, col.names = FALSE)  
