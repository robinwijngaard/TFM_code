...
for (name in names(datasets)) {
  ...
  if(dataset$include){
    ...
    outputFolder <- file.path(getwd(), "output", paste0("manta3-", name))  
    ...
    for(i in 1:length(bamFiles)){
      ...
      resultFile <- file.path(resultFolder, "candidateSV.bedpe")
      if (file.size(resultFile) != 0) {
        ...
        resultData <- resultData[, c(1, 2, 5, 11)]
        ...
        # Edit col names
        colnames(resultData) <- c("Chr", "Start", "End", "CNV.type")
