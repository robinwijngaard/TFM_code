...
processCNVkitBody <- function(launchFile, bedFile, outputFolder,bamsDir, params, fastaFile, refbamsDir){
  ...
  for (cnrFile in cnrFiles){
    ...
    system(paste("python", launchFile, "bintest", file.path(outputFolder, cnrFile), "-t -o", file.path(outputFolder, paste0(sampleName, ".bintest.cnr"))))
    system(paste("python", launchFile, "call", file.path(outputFolder, paste0(sampleName, ".bintest.cnr")), "-m threshold -o", file.path(outputFolder, paste0(sampleName, ".bintest.call.cnr"))))
  }
}
...
for (name in names(datasets)) {
  ...
  if(dataset$include){
    ...
    outputFolder <- file.path(getwd(), "output", paste0("cnvkit4-", name))  
    ...
    # Export results
    calls <- list.files(outputFolder, pattern = ".bintest.call.cnr")
    ...
    for (call in calls){
      sample <- sub(".bintest.call.cnr", "", call)
