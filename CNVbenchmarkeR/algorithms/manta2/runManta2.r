...
for (name in names(datasets)) {
  ...
  if(dataset$include){
    ...
    outputFolder <- file.path(getwd(), "output", paste0("manta2-", name))  
    ...
    for(i in 1:length(bamFiles)){
      ...
      # Process Manta
      processMantaBody(launchFile, bamFile, fastaFile, subsetFolder, bedFile,configFile, params)
      ...
      if (file.size(resultFile) != 0) {
        ...
        # Edit col names
        colnames(resultData) <- c("Chr", "Start", "End", "CNV.type", "Filter", sampleName)
      
        # Select positive cases
        positivePattern <- c("0/1", "1/1")
        positiveCases <- grep(paste(positivePattern, collapse = "|"), resultData[, 6])
        resultData <- resultData[positiveCases, ]
        
        # Add lines to CNVfound
        sampleID <- rep(sampleName, nrow(resultData))
        cnvSample <- cbind(sampleID, resultData)
